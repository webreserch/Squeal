(function (root, factory) {
  ...
  function convertExpression(expr) {
    ...
    // are correctly extracted. Negative-lookbehind avoids matching
    // `obj.super(...)` where super is just a property name.
    expr = mapOutsideStrings(expr, function (chunk) {
      ...
      while ((m = pattern.exec(chunk)) !== null) {
        ...
        out += chunk.slice(lastIdx, m.index) + rendered;
        lastIdx = closeIdx + 1;
    ...
    expr = (function convertTernary(s) {
      ...
      // Skip a Smalltalk single-quoted string, treating `''` as an escaped
      // apostrophe. Returns the index AFTER the closing quote.
      function skipSmalltalkString(str, start) {
        var idx = start + 1;
        while (idx < str.length) {
          if (str.charAt(idx) === "'") {
            if (str.charAt(idx + 1) === "'") { idx += 2; continue; } // escaped
            return idx + 1;
          }
          idx++;
        }
  ...
  function convert(js, options) {
    ...
    for (var idx = 0; idx < lines.length; idx++) {
      ...
      // do { ... } while (cond)  — word-boundary so `doIt();` doesn't match
      if (!transformed && trimmed.match(/^do\b\s*\{?/)) {
        transformed = indent + "[";
